...
//        tree.$container[0].oncontextmenu = function() {return false;};
	...
	_openMenu: function(ctx){
		var data,
			tree = ctx.tree,
			$menu = $(ctx.menu.selector);

		tree.ext.menu.data.node = ctx.node;
		data = $.extend({}, tree.ext.menu.data);

		if( ctx.menu.beforeOpen.call(tree, ctx.originalEvent, data) === false){
			return;
		}
...
//        $menu.position($.extend({my: "left top", at: "left bottom", of: event}, ctx.menu.position));
		$menu
		...
		ctx.menu.open.call(tree, ctx.originalEvent, data);
	},
	_closeMenu: function(ctx){
		var $menu,
            ...
            menu = ctx.menu,
			data = $.extend({}, tree.ext.menu.data);
		if( menu.close.call(tree, ctx.originalEvent, data) === false){
			return;
		}
		$menu = $(menu.selector);
